...
- **Login-Domain sperrt zwei Pfade, nicht drei:** `/admin/` → 403 und `= /` → 404 (Keycloak leitet den Root auf die Admin-Console und damit auf die Admin-Domain weiter). Der OIDC-Auth-Endpunkt `/realms/*/protocol/openid-connect/auth` MUSS auf der Login-Domain erreichbar bleiben – die Admin-Console ist nur ein OIDC-Client im Realm master und authentifiziert wie jeder andere Client gegen die Frontend-URL. Ihn zu sperren bricht jeden Login, nicht nur den der Admin-Console.
